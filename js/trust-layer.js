...
(function () {
    ...
    const CALCULATORS = {
        ...
        'genetic-wealth.html': {
            ...
            input: 'Возраст ребёнка, расходы по возрастным этапам и выбранный уровень расходов.',
        ...
        'wealth.html': {
            purpose: 'сравнивает введённую зарплату с опубликованными официальными ориентирами для России или выбранного субъекта.',
            official: 'Росстат: средняя и медианная начисленная зарплата работников организаций; Правительство России: прожиточный минимум трудоспособного населения. У каждого значения есть период и охват в наборе данных.',
            input: 'Среднемесячная зарплата «на руки» или начисленная до НДФЛ и выбранный субъект. Росстат публикует начисленную зарплату; сумма «на руки» переводится в неё по стандартным правилам НДФЛ для зарплаты.',
            scenario: 'Дополнительной модели нет: процент показывает только разницу между введённой зарплатой и опубликованным ориентиром. Для суммы «на руки» используются годовые ступени НДФЛ 13%, 15%, 18%, 20% и 22% для стандартного трудового дохода.',
            result: 'Разница с доступными официальными значениями. Это не место в распределении и не рейтинг.',
            limitation: 'Калькулятор сравнивает зарплату. Он не измеряет богатство, качество жизни, доход домохозяйства или социальный класс. Перевод суммы «на руки» не учитывает вычеты, специальные режимы, иные доходы, военную службу, самозанятость, ИП или нерезидентство. Метрики без проверенного официального источника не показываются.'
        }
    };
